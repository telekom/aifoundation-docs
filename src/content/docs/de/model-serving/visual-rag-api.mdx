...
**Visual RAG ist ein Beta-Feature. Wenn Sie zu den Ersten gehoren moechten, die es ausprobieren, kontaktieren Sie uns gerne unter: AlFoundation@t-systems.com**

Dieses Dokument stellt unsere Visual RAG API vor und beschreibt, wie Sie Dateien in das VisualRAG-System hochladen, verwalten und einlesen sowie Suchen durchfuehren koennen. Die API ist kompatibel mit dem OpenAI-API-Standard.

Unser Visual RAG indexiert Ihre Datei sowohl als **Text als auch als Bild**. Durch die Kombination beider Indexierungsmethoden vereint es das Beste aus beiden Welten: Die Stabilitaet der Textindexierung sowie die Flexibilitaet und Informationsfuelle der visuellen Indexierung. Im Vergleich zu herkoemmlichen textbasierten RAG-Systemen kann Visual RAG Informationen abrufen, die nur in Grafiken oder Diagrammen vorhanden sind -- etwas, das textbasierte Ansaetze haeufig uebersehen. Darueber hinaus kann Visual RAG in der Regel verschiedene Probleme beim Parsen von Dateien ueberwinden, mit denen textbasierte RAG-Systeme derzeit zu kaempfen haben. Zum Beispiel das Parsen von Tabellen in PDF-Dateien oder das Parsen von Absaetzen in freiem Format wie PPTX.

## Visual RAG API Workflow

Visual RAG unterstuetzt die folgenden Formate: PDF, PPTX, DOCX, HTML und PNG.


<img src="/img/VisionRAG_workflow.jpg" alt="visual_rag_workflow" style="width: 100%; height: auto; display: block;" />

## Einrichtung
Um Zugang zu diesem Feature zu erhalten, muessen Sie sich zunachst an AlFoundation@t-systems.com wenden. Wir werden eine sichere und isolierte Speicherinstanz speziell fuer Ihr Projekt bereitstellen.

Sobald Sie von uns ein "Bereit"-Signal erhalten, koennen Sie Ihre Umgebung wie folgt vorbereiten:
...
Um eine Datei fuer Visual-RAG-Zwecke hochzuladen, geben Sie als "purpose" den Wert "visual-rag" an.
...
Dies gibt ein `FileObject` zurueck, das die Datei-ID der soeben hochgeladenen Datei enthaelt.
...
Der vorgesehene Verwendungszweck der hochgeladenen Datei. Verwenden Sie "visual-rag" fuer Visual RAG.
...
Die ID der abzurufenden Datei. Diese ID kann ueber die Funktion zum Auflisten von Dateien erhalten werden.
...
Loeschen Sie eine bestimmte Datei anhand ihrer ID.
...
Die ID der zu loeschenden Datei. Diese ID kann ueber die Funktion zum Auflisten von Dateien erhalten werden.

## Vector-Store-Operationen

Nachdem Sie den Datei-Upload abgeschlossen haben, ist es an der Zeit, die Dateien in Ihre Vektordatenbank einzulesen. Dazu muessen Sie zunaechst einen Vector Store erstellen.

### Einen Vector Store erstellen
...
Der Name Ihres Vector Stores. Der Name muss unter allen von Ihnen erstellten Vector Stores eindeutig sein.

**chunking_strategy** dictonary: `Required` 

Enthaelt die Modelle fuer die Indexierung auf beide Arten: Der Wert fuer `text_embedding_model` und `vision_embedding_model` sollte der Modellname eines Text-Embedding-Modells bzw. eines Vision-Embedding-Modells Ihrer Wahl sein. 

### Alle Vector Stores auflisten
...
### Eine Datei in einen Vector Store einlesen
...
Die ID Ihres Vector Stores. Diese kann ueber die Funktion `vector_store.list` abgerufen werden.

**file_id** String: `Required`

Die ID Ihrer Datei. Diese kann ueber die Funktion `files.list` abgerufen werden.

**chunking_strategy** dictonary: `Optional` 

Enthaelt die Chunking-Parameter: `chunk_size` definiert die Groesse der Chunks bei der Einlesung. Standardwert ist 1024. `chunk_overlap` definiert die Ueberlappungsgroesse zwischen Chunks. Standardwert ist 100.



### Alle eingelesenen Dateien auflisten
...
### Informationen zu einer eingelesenen Datei abrufen
...
### Eine eingelesene Datei aus einem Vector Store loeschen
...
Die Suchanfrage, mit der Sie den Vector Store durchsuchen.

**extra_body** Dictionary: `Optional` 

- **top_k_texts** integer: Gibt die obersten k Suchergebnisse aus der Textsuche zurueck.
- **top_k_image** integer: Gibt die obersten k Suchergebnisse aus der Bildsuche zurueck.


### Textsuchergebnisse anzeigen:
...
### Bildsuchergebnisse visualisieren:
```py
...
Initialisieren Sie zwei Clients: einen fuer den Abruf im RAG und einen fuer die LLM-Inferenz.

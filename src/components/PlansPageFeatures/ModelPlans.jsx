...
function ModelPlans(props) {
    ...
    return (
        <div className="my-column">
            <span className="font-size-32 font-weight-600">{t('PLANS.MODELS.TITLE')}</span>
            <span>{t('PLANS.MODELS.SUBTITLE')}</span>

            <div className="my-row my-justify-space-between my-align-center">
                <div className="my-input-width-0-5 my-mt-4">
                    <label htmlFor="plan-select" style={{ display: 'block', marginBottom: '4px', fontSize: '12px', color: '#6c6c6c' }}>
                        {t('PLANS.MODELS.SELECTED_RATE_PLAN')}
                    </label>
                    <select
                        id="plan-select"
                    ...
                    >
                        {modelList.map((model) => (
                            <option key={model.packageName} value={model.packageName}>
                                {model.label}
                            </option>
                        ))}
                    </select>
            ...
            {isLoading ? (
                <div className="my-row my-justify-center my-my-6">
                    <span>{t('PLANS.MODELS.LOADING')}</span>
                </div>
            ) : (
                        ...
                        filterSelected={
                            <>
                                {(!filters?.dataSovereigntyFilter || filters?.dataSovereigntyFilter.length === 0) ? (
                                    <span className="plans-tag">{t('PLANS.MODELS.ALL_SERVER_LOCATION')}</span>
                                ) : (
                                    filters.dataSovereigntyFilter.map((item, idx) => (
                                        <span className="plans-tag" key={idx}>
                                            {(item?.name || '') + ' ' + t('PLANS.MODELS.SERVER_LOCATION_SUFFIX')}
                                        </span>
                                    ))
                                )}

                                <span className="plans-tag">

...
export default function PlansRates(props) {
    const { selectedPlanId } = props || {};
    const { t, locale, setLocale } = useMyI18n();

    const dropdownRef = useRef(null);

    // Map incoming plan IDs to a simple key used by rendering logic
    const planKeyById = {
        'Tarif Basic Template': 'basic',
        'Tarif Standard1000 Template': 'std_1000',
        'Tarif Standard2000 Template': 'std_2000',
        'Tarif Standard3000 Template': 'std_3000',
        'Tarif Standard4000 Template': 'std_4000',
    };

    // Derive which single plan (if any) should be shown
    const visiblePlanKey = useMemo(() => {
        if (!selectedPlanId) return undefined; // show all
        return planKeyById[selectedPlanId]; // undefined if not mapped -> show all
    }, [selectedPlanId]);

    // Attach language change listener to the web component
    useEffect(() => {
    ...
    // Ensure the property is in sync for the custom element (good practice)
    useEffect(() => {
        if (dropdownRef.current && dropdownRef.current.value !== locale) {
            dropdownRef.current.value = locale;
        }
    }, [locale]);

    // Helper to decide visibility per row
    const showRow = (key) =>
        !visiblePlanKey || visiblePlanKey === key;
            ...
            <div className="my-column my-my-3">
                <scale-table>
                    <table
                        style={{
                            border:
                                'var(--telekom-spacing-composition-space-01) solid var(--telekom-color-ui-faint) !important',
                        }}
                    >
                        <thead>
                        <tr>
                                ...
                                <td>
                                    <div className="my-row my-gap-2 ">
                                        ...
                                        <scale-tooltip content={t('PLANS.TOOLTIP_AZURE')}>
                                            <scale-tag style={{ position: 'relative', top: '-3px' }}>
                                                {' '}
                                                {t('PLANS.AZURE')}
                                            </scale-tag>
                                        </scale-tooltip>
                ...
                <span className="scl-font-variant-caption my-pmy-4">
          * OTC-Hosted Large Language Models
        </span>
            </div>
        </div>
    );
